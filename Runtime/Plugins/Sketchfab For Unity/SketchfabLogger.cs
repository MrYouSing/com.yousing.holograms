// Generated automatically by MacroCodeGenerator (from "Packages/com.yousing.holograms/Runtime/Plugins/Sketchfab For Unity.ms")
...
namespace Sketchfab
{
	...
	{
		public static string accessTokenKey = "skfb_access_token";
		...
		public void showLoginUi()
		{
			...
			if (_current == null)
			{
				...
				{
					//GUILayout.Label("You're not logged", GUI.skin.label);
					//GUILayout.Space(5);
				...
				{
					GUILayout.Label("Retrieving user data", GUI.skin.label);
				}
			}
			else if (_current.isDisplayable())
			{
				//GUILayout.Label("Logged in as", GUI.skin.label);
		...
		{
			JObject response = Utils.JSONParse(text);
			var jt=response?.SelectToken("access_token")??null;if(jt!=null)
			{
		...
		private string getAvatarUrl(JObject node)
		{
			JArray array = node["avatar"]["images"]as JArray;
			foreach (JObject elt in array)
			{
				if (elt["width"].Value<int>() == 100)
				{
					return elt["url"].Value<string>();
				}
			}
...
#endif
			//.Bilinear(tex, (int)AVATAR_SIZE.x, (int)AVATAR_SIZE.y);
		...
		{
			JObject response = Utils.JSONParse(res);
			//("Login error", "Authentication failed: " + response["error_description"], "OK");
			logout();
		}
		...
		{
			JObject userData = Utils.JSONParse(response);
			_current = new SketchfabProfile(userData["username"].Value<string>(),userData["displayName"].Value<string>(),userData["account"].Value<string>());
		...
		{
			JObject planResponse = Utils.JSONParse(response);
			_current._userCanPrivate = planResponse["canProtectModels"].Value<bool>() ? 1 : 0;
		}
	}
}
#endif
